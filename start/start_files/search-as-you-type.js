...
var searchAsYouTypeConfiguration = {
    ...
    resourcesPath: "/web/fragments/fnrw_top_nav/",
      //"{resourcesPath}",

    // The fully qualified URL to the Ajax responder. 
    // e.g. http://intranet.company.com/search-as-you-type/responder.php
    ajaxResponderUrl: "http://apps.finra.org/TypeAhead/typeahead.ashx",
      //"{ajaxResponderUrl}",
	  
	 suggestUrl: "http://search.finra.org/suggest",
	 moreDetailsUrl: "http://search.finra.org/search",
...
SearchAsYouType.prototype.updateDimensionsAndShadow_ = 
  ...
  if (searchResultsEl) {
    ...
    searchResultsEl.style.left = (x + 0) + "px";
...
SearchAsYouType.prototype.search_ = function(dontDelayShowResults) {
  ...
  if (this.searchCache["_" + this.typedQuery]) { 
	...
	var suggestObject = document.createElement('script');
	suggestObject.src = this.readProperties_(searchAsYouTypeConfiguration.suggestUrl + "?q=" + this.typedQuery) + "&format=rich&access=p&callback=searchAsYouType.addSuggestions&max=9";
	suggestObject.type = "text/javascript";
	suggestObject.charset = "utf-8";
	document.getElementsByTagName('head').item(0).appendChild(suggestObject);
  }
}

SearchAsYouType.prototype.readProperties_ = function(URL) {
	// Read properties from form (finra.org page)
	if (this.inputFieldEl.nextSibling) {
	  var sib = this.inputFieldEl;
	  while (sib = sib.nextSibling) {
		if (sib.type == "hidden") {
			URL += "&" + sib.name + "=" + sib.value;
		}
	  }
	} else {  // Read properties from url (GSA page)
...
SearchAsYouType.prototype.handleAjaxResponse = function(results) {
  ...
  this.joinResults_(results, false);
}

SearchAsYouType.prototype.addSuggestions = function(results) {
	for (var i = 0; i < results.results.length; i++) {
		results.results[i]["content"] = results.results[i].name;
		results.results[i]["style"] = "normal";
		results.results[i]["moreDetailsUrl"] = this.readProperties_(searchAsYouTypeConfiguration.moreDetailsUrl + "?q=" + results.results[i].name);
	}
	results["autocompletedQuery"] = "";
	this.joinResults_(results, true);
}

SearchAsYouType.prototype.joinResults_ = function(results, suggest) {
	if (this.results && this.results.query == this.typedQuery) {
		if (!suggest) {
			if (results.results.length < 10) {
				// TODO Put in taxonomy before suggestions
			} else {
				this.processResults_(results, false);
			}
		} else if (suggest && this.results.results.length < 10) {
			for (var i = 0; i < results.results.length && this.results.results.length < 10; i++) {
				var include = true;
				for (var j = 0; j < this.results.results.length; j++) {
					if (this.results.results[j].name.toLowerCase() == results.results[i].name.toLowerCase()) {
						include = false;
						break;
					}
				}
				if (include) {
					this.results.results.push(results.results[i]);
				}
			}
			if (this.results.results.length > 1) {
				this.results.autocompletedQuery = "";
			} else if (this.results.results.length == 1) {
				this.results.autocompletedQuery = this.results.results[0].content;
			}
		} 
		this.processResults_(this.results, false);
	} else {
		//this.processResults_(results, false);
